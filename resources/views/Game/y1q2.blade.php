      ...
      <div class="modal-content borderornage mt1">
        ...
        <div class="modal-body pt-0 pb-0">
          <div class="container">
              ...
              <div class="col-12">
                <div class="d-flex justify-content-center">
                  ...
                  <div class="row text1">
                    ...
                    <div class="col-12 rec_amount"><p><span id="ReceivableAmt">{{ (isset($product_launch_val->acc_receivable) && $product_launch_val->acc_receivable != '') ? $product_launch_val->acc_receivable : 0}}</span>M</p></div>
                   </div>
                </div>
              </div>
            ...
            @if(isset($amt_deduct) && $amt_deduct != '' && isset($amt_deduct->card_type)  && $amt_deduct->card_type == 'add_coin_q2')
...
<!-- Record Discount -->
    ...
    @if(array_sum($cos_value_1) != 0)
      ...
      @if(array_sum($cos_value_2) != 0)
        ...
        foreach($strategy_selected as $data)       {   
            ...
            $cos_value_2[] = (int) $data->impact_value_cos_y1q3;
          }
        }
      @endphp
      @if(array_sum($cos_value_2) != 0)
        ...
        foreach($strategy_selected as $data)       {   
            ...
            $cos_value_2[] = (int) $data->impact_value_cos_y1q4;
          }
        }
      @endphp
      @if(array_sum($cos_value_2) != 0)
  ...
  <div id="" class="modal-footer noborder dcenter mt-2 footerbtn">
  <input type="text" value="1" id="costotals" style="display:none" >
  <button class="btnstyle table_button"  type="button" onclick="slider_('costotalYear1Q2','cos')"  style="border: none;"><img src="Images/Ok button.svg" alt="" width="50%"></button>
  </div> 
...
Select Product first
  ...
  <div class="modal fade modal-xy" id="salerecord" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true" style="display: none;">
    ...
    <tbody>
      ...
      <tr>
        <td><button type="button" class="quarter">EVENT IMPACT</button></td>
        <td><button type="button" class="tardeamount">-</button></td>
        <td><button type="button" class="tardeamount" id="salesEimpY2">
        @if(($event_impactq2) != 0)
        {{$event_impactq2 }} M
            @else
            - 
        ...
        <td><button type="button" class="tardeamount" id="salesEimpY3">
        @if(($event_impactq3) != 0)
        {{$event_impactq3 }}M
            @else
            - 
        ...
        <td><button type="button" class="tardeamount" id="salesEimpY4">
        @if(($event_impactq4) != 0)
        {{$event_impactq4 }}M
            @else
            - 
  ...
  <div id="" class="modal-footer noborder dcenter mt-2 footerbtn">
    ...
    <button class="btnstyle table_button" type="button" onclick="slider_('salesTotalY1Q2','sales')"  style="border: none;"><img src="Images/Ok button.svg" alt="" width="50%"></button>
    </div>    
</div>
</div>
</div>
  ...
  <div class="modal fade modal-yy" id="RQNS" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    ...
    <tbody>
      <tr>
        ...
        <td><button type="button" class="tardeamount" id="quarterly_sales">{{(isset($product_launch_val->total_sales) && $product_launch_val->total_sales != '') ? $product_launch_val->total_sales : 'x'}} M</button></td>
    ...
    function okbtn1() {
      ...
      if (Radiobtn1 != null) {
        ...
        if (a == 'A') {
          ...
          csat = {{$Events_cards_2['0']->csat }} ;
          bE = {{$Events_cards_2['0']->be }};
          esat = {{$Events_cards_2['0']->esat }};
          var ce=0;
          setTimeout(function () {
          ...
          selectOpt(a,image,div_id)          
          sales = null;
          csat = {{$Events_cards_2['2']->csat }} ;
          bE = {{$Events_cards_2['2']->be }};
          esat = {{$Events_cards_2['2']->esat }};
          var ce=0;
          setTimeout(function () {
      ...
      $("#oknext").attr('onclick','openNext('+"'#Matrixbox','#Backdop2',"+"'"+'optn'+a+"')");
    }
      ...
      $.ajax({
           ...
           data: {a,image,div_id},
            ...
            success: function (response) {
              console.log(response);
                if(a == "A"){
                  document.getElementById('eveImpYear3').innerText = response.eventcos+' M';
                }
                if(a == "B"){
                  document.getElementById('eventq2').innerText = response.eventq2+' M';
                  document.getElementById('eventq3').innerText = response.eventq3+' M';
                  document.getElementById('eventq4').innerText = response.eventq4+' M';

                  document.getElementById('salesEimpY2').innerText = response.eventq2+' M';
                  document.getElementById('salesEimpY3').innerText = response.eventq3+' M';
                  document.getElementById('salesEimpY4').innerText = response.eventq4+' M';
                }
                else if(a == "C"){
                  document.getElementById('eventq2').innerText = response.eventq2+' M';
                  document.getElementById('eventq3').innerText = response.eventq3+' M';
...
function cosTableCal(type){
  var abc_strategy = parseInt(document.getElementById('stgImpYear2').innerHTML);
  if(Number.isInteger(abc_strategy)){
    abc_strategy = parseInt(document.getElementById('stgImpYear2').innerHTML);
  }
  else{
    abc_strategy = 0;
  }
  var costotal = parseInt(document.getElementById('cosYear1Q2').innerHTML) + 
  abc_strategy;
  document.getElementById('costotalYear1Q2').innerHTML = costotal;
...
function saleTableCal(type){
  var abc_sales = parseInt(document.getElementById('salesEimpY2').innerHTML);
  if(Number.isInteger(abc_sales)){
    abc_sales = parseInt(document.getElementById('salesEimpY2').innerHTML);
  }
  else{
    abc_sales = 0;
  }

  var strategy_sales = parseInt(document.getElementById('salesStrY2').innerHTML);
  if(Number.isInteger(strategy_sales)){
    strategy_sales = parseInt(document.getElementById('salesStrY2').innerHTML);
  }
  else{
    strategy_sales = 0;
  }

  var saletotal_2 = parseInt(document.getElementById('salesYear2').innerHTML) + abc_sales + strategy_sales + parseInt(document.getElementById('salestradeSpendY1q2').innerHTML);
...
function tradespendTableCal(){
  ...
  document.getElementById('salestradeSpendY1q2').innerHTML =  parseInt(document.getElementById('tradeSpendY1Q2').innerHTML);
}
...
function  cashcon(){
  ...
  var csh_rec = ((cashreceivable/100)*percentage).toFixed(1);
  console.log('this is cashrec '+ csh_rec);
  document.getElementById('Cashconversion').innerHTML = (((cashreceivable/100)*percentage).toFixed(1)) + 'M';
       ...
       data: {"resouce":10,"type":"resources"},           
        success: function (response) {
          fillblink("finish_circle","parResblink");
            // console.log(response);
            var new_amt = amt.toFixed(1);
            document.getElementById('cash1').innerHTML = new_amt; 
            document.getElementById('cash').innerHTML = new_amt; 
